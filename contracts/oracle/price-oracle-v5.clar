...
(define-read-only (get-price-status (asset (string-ascii 10)))
    (let (
            (price-data (unwrap! (map-get? prices { asset: asset }) err-invalid-asset))
...
(define-read-only (is-price-fresh (asset (string-ascii 10)))
    (match (map-get? prices { asset: asset })
        price-data (let ((age (- stacks-block-height (get last-updated price-data))))
...
(define-read-only (get-price-age (asset (string-ascii 10)))
    (match (map-get? prices { asset: asset })
        price-data (ok (- stacks-block-height (get last-updated price-data)))
        err-invalid-asset
    )
)
